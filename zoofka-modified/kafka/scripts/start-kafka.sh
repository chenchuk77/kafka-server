...
if [ ! -z "$KAFKA_ADVERTISED_HOST_NAME" ]; then
    ...
    if grep -q "^advertised.host.name" $KAFKA_HOME/config/server.properties; then
        sed -r -i "s/#(advertised.host.name)=(.*)/\1=$KAFKA_ADVERTISED_HOST_NAME/g" $KAFKA_HOME/config/server.properties
    else
        echo "advertised.host.name=$KAFKA_ADVERTISED_HOST_NAME" >> $KAFKA_HOME/config/server.properties
    fi
fi
if [ ! -z "$KAFKA_ADVERTISED_PORT" ]; then
    echo "advertised port: $KAFKA_ADVERTISED_PORT"
    if grep -q "^advertised.port" $KAFKA_HOME/config/server.properties; then
        sed -r -i "s/#(advertised.port)=(.*)/\1=$KAFKA_ADVERTISED_PORT/g" $KAFKA_HOME/config/server.properties
    else
        echo "advertised.port=$KAFKA_ADVERTISED_PORT" >> $KAFKA_HOME/config/server.properties
    fi
fi
...
# assign broker-id (docker-compose)
...
if [ ! -z "$KAFKA_ZOOKEEPER_CONNECT" ]; then
    sed -i "s/zookeeper.connect=localhost:2181/zookeeper.connect=$KAFKA_ZOOKEEPER_CONNECT/g" $KAFKA_HOME/config/server.properties
fi
